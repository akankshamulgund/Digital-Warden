...
def find_account(request):
    if request.method == 'POST':
        ...
        blocked_accounts = BlockedAccount.objects.filter(account_id=account_name)
        num_blocked_users = blocked_accounts.count()
        blocked_users = [blocked_account.reporter_name for blocked_account in blocked_accounts]
        return render(request, 'main_app/find_account.html', {'account_name': account_name, 'num_blocked_users': num_blocked_users, 'blocked_users': blocked_users})
    return render(request, 'main_app/find_account.html')
...
def report_account(request):
    if request.method == 'POST':
        ...
        # Create a new BlockedAccount object and save it to the database
        blocked_account = BlockedAccount.objects.create(account_id=account_id, reason=reason, reporter_name=name)
        blocked_account.save()
        return JsonResponse({'message': 'Account reported successfully'})
    return render(request, 'main_app/report_account.html')
